...
public class BattlerGen2 {
  ...
  public BattlerGen2(FormationEntity formationLeft,
                     FormationEntity formationRight, boolean createJson,
                     boolean runWithGUI, Vector gridSize, boolean mirrorEnemy,
                     boolean drawBoard) {
    friendlyUnitList = new ArrayList<>();
    enemyUnitList = new ArrayList<>();
    grid = new Grid(gridSize);
    ...
    try {
      ...
      getFormationFromJson(
          FRIENDLY, HistoryToJson.formationToJson(_formationLeft), mirrorEnemy);
      getFormationFromJson(
          ENEMY, HistoryToJson.formationToJson(_formationRight), mirrorEnemy);
      ...
      history = new History(
          new Formation(friendlyUser, new ArrayList<>(friendlyUnitList)),
          new Formation(enemyUser, new ArrayList<>(enemyUnitList)), this);
  ...
  public static void main(String[] args) {
    ...
    Formation left =
        createFormations.createTestFormation(FRIENDLY, 1, true,
                                             new UnitTypes[] {
                                                 UnitTypes.MY_FIRST_UNIT,
                                             },
                                             1, 10, amountUnitsLeft, unitSlots);
    Formation right = createFormations.createTestFormation(
        ENEMY, amountUnitsLeft + 1, true,
        new UnitTypes[] {
            UnitTypes.MY_FIRST_UNIT,
        },
        1, 10, left.getTotalLevels(), amountUnitsRight, unitSlots);

    new BattlerGen2(new FormationEntity(left), new FormationEntity(right),
                    false, true, gridSize, false, true);
  }

  private void mirrorSide(Unit unit) {
    unit.setGridPosition(new Vector(grid.getGridSize().getX() - 1 -
                                        unit.getGridPosition().getX(),
                                    unit.getGridPosition().getY()));
  }

  // TODO: 28.01.2022 use a pathfinding like algorithm that goes from current
  // node of unit and checks all the neighbours if there is someone, the first
  // one found is considered the closest one
  public Unit findClosestOther(Unit unit, Side sideToCheck,
                               boolean checkIfReachable, boolean includeDead) {
    Unit closestUnit = null;
    Double shortestDistance = -1d;

    ArrayList<Unit> unitsToCheck =
        (sideToCheck == FRIENDLY) ? friendlyUnitList : enemyUnitList;
      ...
      if (unitChecking != unit) {
        if (includeDead || unitChecking.getMyState() != State.DEAD) {
          if (checkIfReachable) {
            ...
            if (!path.isReachable(unit.getGridPosition(),
                                  unitChecking.getGridPosition(), grid)) {
              continue;
            }
          }
          Double temp = unit.getGridPosition().getDistanceFrom(
              unitChecking.getGridPosition());
  ...
  public ActionHistory doTurn(Iterator<Unit> unitIterator, Unit unit,
                              boolean createHistory) {
    ActionHistory actionHistory;
    Vector posBefore = unit.getGridPosition();
    // TODO: 27.01.2022 update the way units die, think about how it should
    // work!
    if (unit.getMyState() != State.ALIVE) {
      ...
      actionHistory = new ActionHistory(Action.DIE, unit, new Unit[0], null,
                                        new Vector[] {unit.getGridPosition()});
      if (createHistory) {
  ...
  private void drawBoard() {
    ...
    for (int row = 0; row < grid.getHeight(); row++) {
      ...
      for (int column = 0; column < grid.getWidth(); column++) {
        ...
        GridCell cell =
            grid.getGridCells()[gridPositionNow.getX()][gridPositionNow.getY()];
