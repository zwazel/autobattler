...
public abstract class Ability extends RoundAffected {
  ...
  public Ability(String title, String description, Unit owner,
                 UsageType costType, int usageCost,
                 AbilityOutputType outputType, int outPutAmount, int cooldown,
                 int range, Side targetSide) {
  ...
  public String getRandomUseMessage(Unit target) {
    ...
    return finalizeString(
        this.useMessages[rand.nextInt(this.useMessages.length)], target);
  }
  ...
  public String getRandomKillMessage(Unit target) {
    Random rand = new Random();
    return finalizeString(
        this.killMessages[rand.nextInt(this.killMessages.length)], target);
  }

  private String finalizeString(String s, Unit target) {
    return s.replace("$targetName", target.getName())
        .replace("$id", "" + target.getID());
  }
  ...
  protected boolean processUse(Unit target) { return use(target); }
