...
public abstract class Unit implements Obstacle, Cloneable {
  ...
  /**
   * the priority of the unit, used to sort the units in the queue and to
   * determine the order of the units
   */
   ...
   * @param symbol   the symbol of the unit, used for printing in the console
   *     (in theory, actually it has never been used yet lol)
   * @param position the position of the unit
   * @param priority the priority of the unit, used for sorting
   * @param type     the type of the unit
   */
  public Unit(long id, int priority, int level, UnitTypes type, char symbol,
   ...
   */
  public Unit(long id, int priority, int level, UnitTypes type, char symbol,
              Vector position, Side side, BattlerGen2 battler, String name) {
  ...
  public Unit(long id, int priority, int level, UnitTypes type, char symbol,
              Vector position, Side side, BattlerGen2 battler) {
    this(id, priority, level, type, symbol, position);
    this.battler = battler;
    this.side = side;
  }
  ...
  public UnitModel getUnitModel() { return new UnitModel(this); }
   ...
   */
  public ActionHistory process() { return run(); }

  /**
   * in this method, the unit will decide what to do.
   *
   * @return the action that the unit did. for example if the unit uses an
   *     ability.
   */
  ...
  protected Vector[] moveTowards(Unit target) {
    if (target != null) {
      Node[] nodes = new FindPath().getNextMoveSteps(
          gridPosition, target.getGridPosition(), battler.getGrid(), speed,
          type.isCanMoveDiagonally());
      if (nodes.length > 0) {
  ...
  // TODO: 17.02.2022 use speed to move as much as possible. get a random
  // destination within reach and move there by using the move method
  ...
  public ActionHistory die(Ability ability) {
    ...
    return new ActionHistory(Action.DIE, this, new Unit[0], null,
                             new Vector[] {this.getGridPosition()});
  }
  ...
  public String toString() {
    return "Unit{"
        ...
        ", priority=" + priority + ", side=" + side + ", level=" + level +
        ", health=" + health + ", energy=" + energy + ", speed=" + speed +
        ", myState=" + myState + ", type=" + type +
  ...
  public Unit clone() {
    try {
      // TODO: copy mutable state here, so the clone can't change the internals
      // of the original
  ...
  public boolean equals(Object o) {
    ...
    if (getID() != unit.getID())
      return false;
    if (getSymbol() != unit.getSymbol())
      return false;
    if (getHealth() != unit.getHealth())
      return false;
    if (getEnergy() != unit.getEnergy())
      return false;
    if (getLevel() != unit.getLevel())
      return false;
    if (getPriority() != unit.getPriority())
      return false;
    if (getSpeed() != unit.getSpeed())
      return false;
    if (getType() != unit.getType())
      return false;
    if (getSide() != unit.getSide())
      return false;
    if (!getName().equals(unit.getName()))
      return false;
    // Probably incorrect - comparing Object[] arrays with Arrays.equals
    if (!Arrays.equals(getAbilities(), unit.getAbilities()))
      return false;
    if (getGridPosition() != null
            ? !getGridPosition().equals(unit.getGridPosition())
            : unit.getGridPosition() != null)
      return false;
    if (getBattler() != null ? !getBattler().equals(unit.getBattler())
                             : unit.getBattler() != null)
      return false;
  ...
  public int hashCode() {
    ...
    result = 31 * result +
             (getGridPosition() != null ? getGridPosition().hashCode() : 0);
