...
public class CreateFormations {
  ...
  public Formation createTestFormation(Side side, long idCounter,
                                       boolean randomPositioning,
                                       UnitTypes[] allowedUnitTypes,
                                       int minLevel, int maxLevel,
                                       int amountUnits, int unitSlots) {
    // get a random number between amountUnits + minlevel and amountUnits +
    // maxLevel
    int totalLevel = (amountUnits + minLevel) +
                     (int)(Math.random() * ((amountUnits + maxLevel) -
                                            (amountUnits + minLevel)));

    return createTestFormation(side, idCounter, randomPositioning,
                               allowedUnitTypes, minLevel, maxLevel, totalLevel,
                               amountUnits, unitSlots);
  }

  public Formation
  createTestFormation(Side side, long idCounter, boolean randomPositioning,
                      UnitTypes[] allowedUnitTypes, int minLevel, int maxLevel,
                      int targetTotalLevel, int amountUnits, int unitSlots) {
    Formation formation;
    ...
    for (int j = 0; j < amountUnits; j++) {
      ...
      UnitTypes type =
          allowedUnitTypes[(int)(Math.random() * allowedUnitTypes.length)];
      ...
      if (tempSlotAmountTaken > unitSlots) {
        ...
        if (isPossibleToFillSlot) {
          while (tempSlotAmountTaken > unitSlots) {
            type = allowedUnitTypes[(int)(Math.random() *
                                          allowedUnitTypes.length)];
            tempSlotAmountTaken = type.getSlotSize() + slotsTaken;
          }
        } else {
          System.err.println(
              "no more possible slots to fill! could not find a unit type of a slot size of " +
              (unitSlots - slotsTaken));
          break;
        }
      }
      ...
      Vector vector = (randomPositioning) ? findFreeRandomSpaceOnSide(side)
                                          : findFreeSpaceOnSide(side);
      if (vector == null) {
        System.err.println("No free space on side " + side);
        break;
      }
      Unit unit = createTestUnit(idCounter++, priorityCounter++, vector, type,
                                 minLevel, maxLevel);
    ...
    while (difference > maxLevelDifference) {
      for (int i = 0; i < indexesOk.length; i++) {
        if (indexesOk[i]) {
          ...
          int randomLevelDifference =
              (int)(Math.random() * (maxLevelDifference + 1)) + 1;
    ...
    formation =
        new Formation(new User("TestUser_" + side, "TestUser_" + side), units);
  ...
  public Unit createTestUnit(long id, int priority, Vector position,
                             UnitTypes type, int minLevel, int maxLevel) {
    // get random number between min and max level
    int level = minLevel + (int)(Math.random() * (maxLevel - minLevel));

    SimpleUnit simpleUnit = new SimpleUnit(id, priority, level, position, type,
                                           getRandomUnitName());
   ...
   * @param counter keep track of how many times this method has been called, to
   *     prevent endless loops. stops after going through all spaces on the side
   * @return a vector with the coordinates of the free space on the side or null
  ...
  public String getRandomUnitName() {
    String[] unitNames = new String[] {"Tim",
                                       "Ash",
                                       "Phillip",
                                       "Moritz",
                                       "Mütz",
