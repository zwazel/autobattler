...
public class HistoryToJson {
  ...
  public static String toJson(History history) {
    Gson gson = new Gson();
    String json = "{\"gridSize\":" +
                  gson.toJson(history.getBattler().getGrid().getGridSize()) +
                  ",";
  ...
  private static String actionHistoryToJson(ActionHistory actionHistory) {
    ...
    json.append("\"user\":{"
                + "\"id\":")
        .append(actionHistory.getUser().getID())
        .append(",")
        .append("\"side\":\"")
        .append(actionHistory.getUser().getSide())
        .append("\",")
        .append("\"health\":")
        .append(actionHistory.getUser().getHealth())
        .append(",")
        .append("\"energy\":")
        .append(actionHistory.getUser().getEnergy())
        .append("}");
    ...
    json.append(",\"ability\":{")
        .append((actionHistory.getAbility() == null)
                    ? "\"title\":\"undefined\""
                    : "\"title\":\"" + actionHistory.getAbility().getTitle() +
                          "\""
                          + ",\"targetSide\":\"" +
                          actionHistory.getAbility().getTargetSide() + "\""
                          + ",\"outPutType\":"
                          + "\"" + actionHistory.getAbility().getOutputType() +
                          "\","
                          + "\"outPutAmount\":" +
                          actionHistory.getAbility().getOutPutAmount())
        .append("}");

    json.append(",\"type\":"
                + "\"")
        .append(actionHistory.getActionType())
        .append("\"");
  ...
  public static String unitToJson(Unit unit) {
    ...
    json.append("\"id\":")
        ...
        .append("\"")
        .append(",\"priority\":")
        .append(unit.getPriority())
        .append(",\"position\":")
        .append(gson.toJson(unit.getGridPosition()))
        .append(",\"level\":")
