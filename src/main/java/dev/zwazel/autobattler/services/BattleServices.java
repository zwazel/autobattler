...
public class BattleServices {
  ...
  public ResponseEntity<UnitTypeWithInfo[]> getUnitTypes() {
    ...
    UnitTypeWithInfo[] unitTypeWithInfos =
        new UnitTypeWithInfo[unitTypes.length];
  ...
  public ResponseEntity<String> getFightHistory(@PathVariable long formationId,
    ...
    if (valid) {
      ...
      // TODO: 08.02.2022 IMPLEMENT SOMETHING LIKE MATCHMAKING TO GET THE ENEMY
      // USER
      Optional<User> user2 = userRepository.findByUsername(username);

      if (user1.isPresent() && user2.isPresent()) {
        Optional<FormationEntity> formationEntity =
            formationEntityRepository.findById(formationId);
        // TODO: 08.02.2022 FIND ENEMY FORMATION

        if (formationEntity.isPresent()) {
          CreateFormations createFormations =
              new CreateFormations(userGridSize, true);
          ...
          int randomNumber =
              (int)(Math.random() * (maxAmountUnits - minAmountUnits + 1)) +
              minAmountUnits;
          Formation formation = createFormations.createTestFormation(
              Side.ENEMY, 1, true,
              new UnitTypes[] {
                  UnitTypes.MY_FIRST_UNIT,
              },
              formationEntity.get().getMinLevel(),
              formationEntity.get().getMaxLevel(),
              formationEntity.get().getTotalLevel(), randomNumber, unitSlots);

          FormationEntity formationEntityEnemyRandom =
              new FormationEntity(formation);

          BattlerGen2 battler =
              new BattlerGen2(formationEntity.get(), formationEntityEnemyRandom,
                              false, false, gridSize, true, false);
          History history = battler.getHistory();

          if (history == null) {
            // return error, history doesn't exist
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body("History doesn't exist");
          } else {
      ...
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
          .body("Unauthorized, JWT is invalid!");
    }
  }
}
