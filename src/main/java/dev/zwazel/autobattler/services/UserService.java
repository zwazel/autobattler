...
public class UserService {
  ...
  public UserService(UserRepository userRepository,
                     ...
                     FormationUnitTableRepository formationUnitTableRepository,
                     JwtUtils jwtUtils) {
  ...
  private Optional<User> getUserWithJWT(UserRepository userRepository,
  ...
  public List<FormationOnly> getAllFormations(HttpServletRequest request) {
    ...
    if (userOptional.isPresent()) {
      ...
      List<FormationEntity> formationEntities =
          formationEntityRepository.findAllByUserOrderById(user);
  ...
  public List<UnitOnly> getAllUnits(HttpServletRequest request) {
    ...
    if (userOptional.isPresent()) {
      ...
      List<UnitModel> unitModels =
          unitModelRepository.findAllByUserOrderByLevel(user);
  ...
  public ResponseEntity<UnitOnly> updateUnit(@RequestBody SimpleUnit simpleUnit,
                                             HttpServletRequest request) {
    Optional<User> userOptional = getUserWithJWT(userRepository, request);

    if (userOptional.isPresent()) {
      User user = userOptional.get();
      Optional<UnitModel> unitModelOptional =
          unitModelRepository.findById(simpleUnit.getId());
      if (unitModelOptional.isPresent()) {
        ...
        return ResponseEntity.ok(
            FormationServiceTemplate.getUnitOnly(unitModel));
      }
    }
  ...
  public ResponseEntity<UnitOnly> addUnit(@RequestBody SimpleUnit simpleUnit,
                                          HttpServletRequest request) {
    Optional<User> userOptional = getUserWithJWT(userRepository, request);
  ...
  deleteFormationFromUser(@RequestBody FormationIdOnly formationID,
                          HttpServletRequest request) {
    Optional<User> userOptional = getUserWithJWT(userRepository, request);
    ...
    if (userOptional.isPresent()) {
      ...
      Optional<FormationEntity> formationEntityOptional =
          formationEntityRepository.findById(formationID.formationID);
  ...
  // TODO: 19.02.2022 - SET A LIMIT OF HOW MANY UNITS PER FORMATION A USER CAN
  // HAVE
  @PostMapping(path = "/addFormation")
  public ResponseEntity<?> setFormationForUser(
      @RequestBody FormationServiceTemplate formationServiceTemplate,
      HttpServletRequest request) {
    Optional<User> userOptional = getUserWithJWT(userRepository, request);
    if (userOptional.isPresent()) {
      User user = userOptional.get();
      ...
      if (formationServiceTemplate.getId() == null) {
        ...
        if ((MAXIMUM_AMOUNT_FORMATIONS < 0) ||
            (amountFormations < MAXIMUM_AMOUNT_FORMATIONS)) {
          try {
            FormationEntity formationEntity =
                formationServiceTemplate.getFormationEntity(
                    user, unitModelRepository);
            ...
            if (slotsAreValid(formationEntity)) {
              if (formationDoesNotExist(formationEntity, user)) {
                ...
                return ResponseEntity.ok(
                    FormationServiceTemplate.getFormationOnly(formationEntity));
              } else {
              ...
              return ResponseEntity.badRequest().body(
                  "Slots are not valid! Max: " + MAXIMUM_SLOTS_PER_FORMATION);
            }
          } catch (UnknownUnitType | NotFoundException e) {
            e.printStackTrace();
          }
        } else {
          return ResponseEntity.badRequest().body("You can't have more than " +
                                                  MAXIMUM_AMOUNT_FORMATIONS +
                                                  " formations");
        }
      } else {
        // update formation
        Optional<FormationEntity> formationEntityOptional =
            formationEntityRepository.findById(
                formationServiceTemplate.getId());
        if (formationEntityOptional.isPresent()) {
          FormationEntity formationEntity = formationEntityOptional.get();
          if (formationEntity.getUser().getId() == user.getId()) {
            try {
              FormationEntity newFormationEntity =
                  formationServiceTemplate.getFormationEntity(
                      user, unitModelRepository);

              // add every new unit to the formation
              for (FormationUnitTable unitNew :
                   newFormationEntity.getFormationUnitTable()) {
                boolean found = false;
                for (FormationUnitTable unitOld :
                     formationEntity.getFormationUnitTable()) {
              ...
              if (unitsToRemove.size() > 0) {
                unitsToRemove.forEach(
                    formationEntity.getFormationUnitTable()::remove);
              ...
              if (slotsAreValid(formationEntity)) {
                if (formationDoesNotExist(formationEntity, user)) {
                  ...
                  formationEntity =
                      formationEntityRepository.save(formationEntity);
                  return ResponseEntity.ok(
                      FormationServiceTemplate.getFormationOnly(
                          formationEntity));
                } else {
                  System.err.println("formation already exists");
                  return ResponseEntity.badRequest().body(
  ...
  private boolean formationDoesNotExist(FormationEntity formation, User user) {
    ...
    for (FormationUnitTable formationUnitTable :
         formation.getFormationUnitTable()) {
      List<FormationEntity> formationEntitySame =
          formationEntityRepository.findAllByFormationUnitTableAndUser(
              formationUnitTable, user);
